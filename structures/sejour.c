...
int lectureSejours(sejour *courant) {
	printf("test\n");
	int nbSejours = 1, jour, mois, annee, idSuivant, idEmplacement;
	...
	while(!feof(fSejour)) {
		fscanf(fSejour, "%d %d %d %2d %2d %4d %f %d %d %d", &courant->id, &courant->formule, &courant->nbPersonnes, &jour, &mois, &annee, &courant->idClient, &courant->prix, &idSuivant, &idEmplacement); 
		...
		strcpy(courant->date, tmpDate);
		printf("%d %d %d %s %6.2f\n", courant->id, courant->formule, courant->nbPersonnes, courant->date, courant->prix);
		nbSejours++;
		sejourSuivant = malloc(sizeof(sejour));
		courant->nxtSej = sejourSuivant;
